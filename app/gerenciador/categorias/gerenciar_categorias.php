...
$sql_categorias = "SELECT * FROM categorias WHERE empresa_id = ? ORDER BY nome ASC";
$stmt = $pdo->prepare($sql_categorias);
$stmt->execute([$empresa_id]);
$categorias = $stmt->fetchAll();


$sql_total = "SELECT COUNT(id) FROM categorias WHERE empresa_id = ?";
$stmt = $pdo->prepare($sql_total);
$stmt->execute([$empresa_id]);
$total_categorias = $stmt->fetchColumn() ?? 0;
...
<body class="flex min-h-screen flex-col bg-gray-100 dark:bg-gray-900 transition-colors duration-500">
    <main class="flex flex-row gap-6 max-h-screen max-sm:flex-col">
        <aside
            class="w-48 bg-[#004b8d] dark:bg-gray-900 shadow-md h-screen flex flex-col max-sm:w-full max-sm:h-full max-sm:flex-row transition-colors duration-500">
            ...
            <nav
                class="sm:hidden flex flex-row items-center w-full overflow-x-auto text-sm text-white font-['Poppins']">
                <a href="gerenciador.php"
                    class="p-2 whitespace-nowrap flex items-center gap-1 <?= basename($_SERVER['PHP_SELF']) == 'gerenciador.php' ? 'bg-white/20 dark:bg-gray-800' : 'hover:bg-white/10 dark:hover:bg-gray-800' ?>">
                    <i class="fas fa-home text-xs"></i> Inicio
                </a>
                <a href="../gerenciador/despesas/gerenciar_despesas.php"
                    class="p-2 whitespace-nowrap flex items-center gap-1 <?= basename($_SERVER['PHP_SELF']) == 'gerenciar_despesas.php' ? 'bg-white/20 dark:bg-gray-800' : 'hover:bg-white/10 dark:hover:bg-gray-800' ?>">
                    <i class="fas fa-arrow-down text-xs"></i> Despesas
                </a>
                <a href="../gerenciador/receitas/gerenciar_receitas.php"
                    class="p-2 whitespace-nowrap flex items-center gap-1 <?= basename($_SERVER['PHP_SELF']) == 'gerenciar_receitas.php' ? 'bg-white/20 dark:bg-gray-800' : 'hover:bg-white/10 dark:hover:bg-gray-800' ?>">
                    <i class="fas fa-arrow-up text-xs"></i> Receitas
                </a>
                <a href="../gerenciador/categorias/gerenciar_categorias.php"
                    class="p-2 whitespace-nowrap flex items-center gap-1 <?= basename($_SERVER['PHP_SELF']) == 'gerenciar_categorias.php' ? 'bg-white/20 dark:bg-gray-800' : 'hover:bg-white/10 dark:hover:bg-gray-800' ?>">
                    <i class="fas fa-tags text-xs"></i> Categorias
                </a>
                <a href="/metas/gerenciar_metas.php"
                    class="p-2 whitespace-nowrap flex items-center gap-1 <?= basename($_SERVER['PHP_SELF']) == 'gerenciar_metas.php' ? 'bg-white/20 dark:bg-gray-800' : 'hover:bg-white/10 dark:hover:bg-gray-800' ?>">
                    <i class="fas fa-bullseye text-xs"></i> Metas
                </a>
                <button id="openSettingsModalSm"
                    class="p-2 whitespace-nowrap flex items-center gap-1 hover:bg-white/10 dark:hover:bg-gray-800">
        ...
        <div class="flex-1 flex-col p-6 overflow-y-auto max-h-screen">
            <div class="flex flex-row justify-between items-center flex-wrap">
                <h1
                    class="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-6 max-sm:text-xl max-sm:mb-3 transition-colors duration-500 flex items-center gap-2">
                    <i class="fas fa-tags text-blue-500"></i>
                    Gerenciamento de Categorias
                </h1>

                <div class="flex gap-3 max-sm:w-full max-sm:justify-end max-sm:mb-4">
                    <a href="adicionar_categorias.php"
            ...
            <div
                class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-6 border-l-4 border-yellow-500 animate__animated animate__fadeInDown transition-colors duration-500">
                <h4
                    class="font-['Poppins'] text-lg text-gray-600 dark:text-gray-300 font-semibold mb-2 transition-colors duration-500 flex items-center gap-2">
                    <i class="fas fa-chart-bar text-yellow-500"></i>
                    Total de Categorias Cadastradas
                </h4>
                <p
                    class="font-['Poppins'] text-3xl text-yellow-600 dark:text-yellow-500 font-bold flex items-center gap-2">
                    <i class="fas fa-hashtag text-2xl"></i>
                    <?= number_format($total_categorias, 0, ',', '.'); ?>
                </p>
            </div>

            <div
                class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 animate__animated animate__fadeInUp transition-colors duration-500">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 flex-wrap">

                    <div class="w-full md:w-64 relative">
                        <i class="fas fa-search absolute left-3 top-3 text-gray-400"></i>
                        <input id="search" type="text" placeholder="Buscar por nome da categoria..."
                    ...
                    <div class="flex gap-2 max-sm:w-full max-sm:justify-between">
                        <button
                            class="sort px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 max-sm:flex-1 transition-colors duration-500 flex items-center gap-2"
                            data-sort="nome">
                ...
                <div id="categorias-list" class="overflow-x-auto">
                    <table class="min-w-full">
                        <thead>
                            <tr
                                class="bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-200 uppercase text-xs leading-normal transition-colors duration-500">
                                <th class="py-3 px-6 text-left sort whitespace-nowrap flex items-center gap-2"
                                    data-sort="nome">
                        ...
                        <tbody class="list text-gray-600 dark:text-gray-300 text-sm font-light">
                            <?php foreach ($categorias as $categoria): ?>
                                <tr
                                    class="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-500">
                                    <td class="py-3 px-6 text-left nome whitespace-nowrap flex items-center gap-2">
                                        <i class="fas fa-folder text-blue-500"></i>
                                        <?= htmlspecialchars($categoria['nome']) ?>
                                    </td>
                                    <td class="py-3 px-6 text-center whitespace-nowrap">
                                        <div class="flex item-center justify-center gap-2">
                                            <a href="editar_categorias.php?id=<?= $categoria['id'] ?>"
                                            ...
                                            <a href="#"
                                                ...
                                                title="Excluir"
                                                onclick="confirmarExclusao(<?= $categoria['id'] ?>); return false;">
                                ...
                                <tr>
                                    <td colspan="3" class="py-8 px-6 text-center text-sm text-gray-500 dark:text-gray-400">
                                        <div class="flex flex-col items-center gap-3">
                                            ...
                                            <span>Nenhuma categoria encontrada. Adicione uma nova categoria!</span>
                                        </div>
                                    </td>
                                </tr>
                            <?php endif; ?>
                    ...
                    <div
                        class="flex justify-between items-center mt-4 flex-wrap text-gray-600 dark:text-gray-300 transition-colors duration-500">
                        <div class="text-sm max-sm:mb-2 flex items-center gap-1">
                            <i class="fas fa-list"></i>
                            Mostrando <span class="page-start font-semibold"></span> a <span
                                class="page-end font-semibold"></span> de <span class="list-total font-semibold"></span>
                            categorias
                        </div>
    ...
    <script src="../../assets/js/categorias_listJS.js"></script>
    <script>
        ...
        function confirmarExclusao(categoriaId) {
            Swal.fire({
                title: 'Tem certeza?',
                text: "Você não poderá reverter esta ação! Todas as despesas e receitas ligadas a esta categoria serão definidas como 'Sem Categoria'.",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    window.location.href = 'excluir_categoria.php?id=' + categoriaId;
                }
            });
        }
