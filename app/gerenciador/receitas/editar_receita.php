...
if ($id_receita > 0) {
    $sql_receita = "SELECT * FROM receitas WHERE id = ? AND empresa_id = ?";
    $stmt = $pdo->prepare($sql_receita);
    $stmt->execute([$id_receita, $empresa_id]);
    $receita_dados = $stmt->fetch();
...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    try {
        ...
        $stmt->execute([$descricao, $valor, $data, $categoria_id, $id_receita, $empresa_id]);
        
        $pdo->commit();
        header("Location: gerenciar_receitas.php?status=success");
        exit();
    } catch (Exception $e) {
        $pdo->rollBack();
        echo "Erro ao editar receita: " . $e->getMessage();
    }
}
    ...
    <main class="container mx-auto py-8 px-4">
        <div class="flex justify-center">
            <div class="w-full max-w-md">
                ...
                <form action="editar_receita.php?id=<?= $id_receita ?>" method="post" 
                    ...
                    <div class="w-full">
                        ...
                        <input type="text" id="descricao" name="descricao" value="<?= htmlspecialchars($receita_dados['descricao'] ?? '') ?>" required
                    ...
                    <div class="w-full">
                        ...
                        <div class="relative">
                            <input type="number" step="0.01" id="valor" name="valor" value="<?= htmlspecialchars($receita_dados['valor'] ?? '') ?>" required
                    ...
                    <div class="w-full">
                        ...
                        <select id="categoria_id" name="categoria_id" required
                            ...
                            <option value="<?= $categoria['id']; ?>" class="dark:bg-gray-700"
                                <?= (isset($receita_dados['categoria_id']) && $receita_dados['categoria_id'] == $categoria['id']) ? 'selected' : '' ?>>
                    ...
                    <div class="w-full">
                        ...
                        <input type="date" id="data" name="data" value="<?= htmlspecialchars($receita_dados['data'] ?? date('Y-m-d')) ?>" required
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg 
                                   focus:outline-none focus:ring-2 focus:ring-primary-gain focus:border-primary-gain 
                                   dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100 transition-all">
                    </div>
