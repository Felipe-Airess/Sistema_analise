...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    try {
        ...
        $sql = "INSERT INTO receitas (descricao, valor, data, empresa_id, categoria_id) VALUES (?, ?, ?, ?, ?)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$descricao, $valor, $data, $empresa_id, $categoria_id]);
        $pdo->commit();
        header("location:gerenciar_receitas.php");
        exit();
    } catch (Exception $e) {
        $pdo->rollBack();
        echo "Erro ao adicionar receita: " . $e->getMessage();
    }
}
            ...
            <div class="w-full max-w-md">
                ...
                <form method="POST" 
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="number" name="valor" step="0.01" min="0.01" required placeholder="Ex: 100.00"
                    ...
                    <div>
                        ...
                        <select name="categoria_id" required
                            ...
                            <option value="<?= $categoria['id'] ?>" class="dark:bg-gray-700"><?= htmlspecialchars($categoria['nome']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Data</label>
                    ...
                    <div class="flex space-x-4 mt-4">
                        <button type="button" onclick="window.history.back()"
                            class="w-1/2 bg-gray-400 text-white py-3 px-4 rounded-lg font-semibold
